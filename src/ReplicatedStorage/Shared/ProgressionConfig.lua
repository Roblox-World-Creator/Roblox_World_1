local config = {
	...
	MeleeCooldown = 0.45,
	MeleeComboReset = 1.1,
	LevelMoveSpeedBonus = 0.18,
	StartingJumpPower = 50,
	LevelJumpPowerBonus = 0.34,
	AimAssistPixels = 105,
	MinimumProjectileSplashRadius = 7,
	RangedCooldown = 0.55,
	RangedProjectileSpeed = 180,
	MeleeCombo = {
		{DamageMultiplier = 1, Cooldown = 0.34, HitDelay = 0.1, Stun = 0.12, Knockback = 5},
		{DamageMultiplier = 1.05, Cooldown = 0.34, HitDelay = 0.1, Stun = 0.14, Knockback = 6},
		{DamageMultiplier = 1.15, Cooldown = 0.4, HitDelay = 0.12, Stun = 0.18, Knockback = 8},
		{DamageMultiplier = 1.6, Cooldown = 0.72, HitDelay = 0.16, Stun = 0.5, Knockback = 42},
	},
	...
	Abilities = {
		...
		LightningBolt = {DisplayName = "Lightning Bolt", Description = "Launch a visible lightning spear that zaps its target and splashes nearby mobs.", Element = "Lightning", EffectProfile = "LightningBolt01", CastType = "Projectile", Targeting = "Point", RequiredLevel = 1, RequiredEvolution = 0, Cooldown = 2.6, EnergyCost = 12, Damage = 58, Range = 100, Radius = 8, LocalRadius = 12, ProjectileSpeed = 135},
	...
	MotionPowers = {
		PowerDash = {DisplayName = "Backstrike Blink", Category = "Mobility", RequiredLevel = 1, Cooldown = 1.25, StaminaCost = 18, Description = "Blink through the nearest enemy ahead and turn to attack from behind."},
	...
	for index, id in ipairs(names) do
		if not config.Abilities[id] then
			...
			config.Abilities[id] = {
				...
				RequiredLevel = level, RequiredEvolution = math.min(3, math.floor(level / 30)),
				...
				AscendantPulses = level >= 60 and math.min(3, math.floor((level - 40) / 20)) or 0,
				AscendantPulseDelay = 0.28,
			}
